...
void UETPRTSComponent::Input_LookStick(const FInputActionValue& InputActionValue)
{
	...
	if (Value.X != 0.0f)
	{
		Pawn->AddControllerYawInput(Value.X * ETPHero::LookYawRate * World->GetDeltaSeconds());
	}

	if (Value.Y != 0.0f)
	{
		Pawn->AddControllerPitchInput(Value.Y * ETPHero::LookPitchRate * World->GetDeltaSeconds());
	}
}
